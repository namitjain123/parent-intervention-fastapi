from sqlalchemy import Column, Integer, String, Boolean, ForeignKey, DateTime, Text
from sqlalchemy.orm import relationship
from datetime import datetime, timezone
from app.db.base import Base


# =========================
# USER TABLE
# =========================
class User(Base):
    __tablename__ = "users"

    id = Column(Integer, primary_key=True, index=True)
    azure_id = Column(String, unique=True, index=True, nullable=False)
    name = Column(String, nullable=False)
    email = Column(String, nullable=False)

    # Questionnaire flags
    pre_questionnaire_completed = Column(Boolean, default=False)
    post_questionnaire_completed = Column(Boolean, default=False)

    # Class A / B  (derived flow: "A" = normal, "B" = delayed)
    user_class = Column(String, nullable=True)  # "A" or "B"

    # grade + teacher captured from the pre-questionnaire
    grade = Column(String, nullable=True)         # "7" | "8" | "9"
    teacher_code = Column(String, nullable=True)  # raw teacher value from REDCap
    teacher_name = Column(String, nullable=True)  # resolved teacher name

    # Episode tracking
    current_episode = Column(Integer, default=0)

    # Activity tracking
    created_at = Column(DateTime, default=lambda: datetime.now(timezone.utc))
    last_activity_at = Column(DateTime, default=lambda: datetime.now(timezone.utc))

    # Reminder system
    last_reminder_sent_at = Column(DateTime, nullable=True)
    reminder_count = Column(Integer, default=0)
    midway_reminder_sent_at = Column(DateTime, nullable=True)
    progress_reminder_count = Column(Integer, default=0)

    # Pre-questionnaire nudge: registered but never started the pre-questionnaire
    last_pre_survey_reminder_sent_at = Column(DateTime, nullable=True)
    pre_survey_reminder_count = Column(Integer, default=0)

    # Post-test nudge: all episodes done but the post-questionnaire isn't.
    # Set once, the moment current_episode crosses the last episode.
    all_episodes_completed_at = Column(DateTime, nullable=True)
    last_post_survey_reminder_sent_at = Column(DateTime, nullable=True)
    post_survey_reminder_count = Column(Integer, default=0)

    # =========================
    # DELAYED SURVEY SYSTEM (CLASS B)
    # =========================
    delayed_survey_unlocked = Column(Boolean, default=False)
    delayed_survey_completed = Column(Boolean, default=False)
    delayed_unlock_at = Column(DateTime, nullable=True)

    lock_email_sent_at = Column(DateTime, nullable=True)
    unlock_email_sent_at = Column(DateTime, nullable=True)

    # Relationships
    progress = relationship("UserEpisodeProgress", back_populates="user")


# =========================
# EPISODES TABLE
# =========================
class Episode(Base):
    __tablename__ = "episodes"

    id = Column(Integer, primary_key=True, index=True)
    episode_number = Column(Integer, unique=True, nullable=False)

    title = Column(String, nullable=False)
    description = Column(Text, nullable=True)

    audio_url = Column(String, nullable=False)

    # transcript support
    transcript_url = Column(String, nullable=True)
    transcript_text = Column(Text, nullable=True)

    quiz_json = Column(Text, nullable=True)

    is_published = Column(Boolean, default=True)

    created_at = Column(DateTime, default=lambda: datetime.now(timezone.utc))

    progress = relationship("UserEpisodeProgress", back_populates="episode")


# =========================
# USER PROGRESS TABLE
# =========================
class UserEpisodeProgress(Base):
    __tablename__ = "user_episode_progress"

    id = Column(Integer, primary_key=True, index=True)

    user_id = Column(Integer, ForeignKey("users.id"), nullable=False)
    episode_id = Column(Integer, ForeignKey("episodes.id"), nullable=False)

    started_at = Column(DateTime, nullable=True)
    completed = Column(Boolean, default=False)
    completed_at = Column(DateTime, nullable=True)

    time_spent_seconds = Column(Integer, default=0)

    user = relationship("User", back_populates="progress")
    episode = relationship("Episode", back_populates="progress")


# =========================
# QUIZ RESPONSES
# =========================
class UserQuizResponse(Base):
    __tablename__ = "user_quiz_responses"

    id = Column(Integer, primary_key=True, index=True)

    user_id = Column(Integer, ForeignKey("users.id"), nullable=False)
    episode_id = Column(Integer, ForeignKey("episodes.id"), nullable=False)

    question_text = Column(Text, nullable=True)
    response_text = Column(Text, nullable=True)
    skipped = Column(Boolean, default=False)

    created_at = Column(DateTime, default=lambda: datetime.now(timezone.utc))


# =========================
# REACTIONS (EMOJI + TIMESTAMP)
# =========================
class UserEpisodeReaction(Base):
    __tablename__ = "user_episode_reactions"

    id = Column(Integer, primary_key=True, index=True)

    user_id = Column(Integer, ForeignKey("users.id"), nullable=False)
    episode_id = Column(Integer, ForeignKey("episodes.id"), nullable=False)

    emoji = Column(String, nullable=False)

    # IMPORTANT for transcript sync UX
    audio_timestamp_seconds = Column(Integer, nullable=False)

    created_at = Column(DateTime, default=lambda: datetime.now(timezone.utc))
